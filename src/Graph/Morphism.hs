...
type NodeAction a = (Maybe Int, Maybe Int)
type EdgeAction a = (Maybe Int, Maybe Int)
...
showNodeAction na = case na of
    ...
    (Just n, Nothing) -> show n ++ " ==> __\n"
    (Nothing, Just n) -> show n ++ " ==> __\n"
    (Just ln, Just gn) ->
        show ln ++ " ==> " ++ show gn ++ "\n"
...
addNodeAction :: Int -> Int -> Morphism a b -> Morphism a b
